let dados = [{
    id : '0001',
    nome : 'Bulbasaur',
    tipo : 'Grama e Veneno',
    fraqueza : 'Fogo, Gelo e Psíquico',
    descricao : 'Por algum tempo após seu nascimento, ele utiliza os nutrientes que estão contidos na semente em seu dorso para crescer.',
    imagem : 'https://www.pokemon.com/static-assets/content-assets/cms2/img/pokedex/full/001.png',
    link : 'https://www.pokemon.com/br/pokedex/bulbasaur'
},
{
    id : '0002',
    nome : 'Charmander',
    tipo : 'Fogo',
    fraqueza: 'Água e Pedra',
    descricao : 'A chama em sua cauda mostra a força de sua força vital. Se Charmander estiver fraco, a chama também queimará fracamente.',
    imagem : 'https://www.pokemon.com/static-assets/content-assets/cms2/img/pokedex/full/004.png',
    link : 'https://www.pokemon.com/br/pokedex/charmander'
},
{
    id : '0003',
    nome : 'Squirtle',
    tipo : 'Água',
    fraqueza: 'Grama e Eletricidade',
    descricao : 'Após o nascimento, suas costas incha e endurece formando uma concha. Ele espalha uma espuma potente pela boca.',
    imagem : 'https://www.pokemon.com/static-assets/content-assets/cms2/img/pokedex/full/007.png',
    link : 'https://www.pokemon.com/br/pokedex/squirtle',
    tag : 'agua'
},
{
    id : '0004',
    nome : 'Pikachu',
    tipo : 'Elétrico',
    fraqueza: 'Ground',
    descricao : 'Quando está irritado, descarrega imediatamente a energia armazenada nas bolsas em suas bochechas.',
    imagem : 'https://www.pokemon.com/static-assets/content-assets/cms2/img/pokedex/full/025.png',
    link : 'https://www.pokemon.com/br/pokedex/pikachu'
},
{
    id : '0005',
    nome : 'Psyduck',
    tipo : 'Água',
    fraqueza: 'Grama e Eletricidade',
    descricao : 'É constantemente assolado por uma dor de cabeça. Quando a dor de cabeça fica intensa, ela começa a usar poderes misteriosos.',
    imagem : 'https://www.pokemon.com/static-assets/content-assets/cms2/img/pokedex/full/054.png',
    link : 'https://www.pokemon.com/br/pokedex/psyduck'
},
{
    id : '0006',
    nome : 'Cubone',
    tipo : 'Ground',
    fraqueza: 'Água, Grama e Gelo',
    descricao : 'Quando a memória de sua falecida mãe o leva às lágrimas, seus gritos ecoam tristemente dentro do crânio que ele usa na cabeça.',
    imagem : 'https://www.pokemon.com/static-assets/content-assets/cms2/img/pokedex/full/104.png',
    link : 'https://www.pokemon.com/br/pokedex/cubone'
},
{
    id : '0007',
    nome : 'Mew',
    tipo : 'Psíquico',
    fraqueza: 'Fantasma e Escuridão',
    descricao : 'Quando visto através de um microscópio, o cabelo curto, fino e delicado deste Pokémon pode ser visto.',
    imagem : 'https://www.pokemon.com/static-assets/content-assets/cms2/img/pokedex/full/151.png',
    link : 'https://www.pokemon.com/br/pokedex/mew'
},
{
    id : '0008',
    nome : 'Mewtwo',
    tipo : 'Psíquico',
    fraqueza: 'Fantasma e Escuridão',
    descricao : 'Seu DNA é quase igual ao de Mew. No entanto, seu tamanho e disposição são muito diferentes.',
    imagem : 'https://www.pokemon.com/static-assets/content-assets/cms2/img/pokedex/full/150.png',
    link : 'https://www.pokemon.com/br/pokedex/mewtwo'
},
{
    id : '0009',
    nome : 'Togepi',
    tipo : 'Fada',
    fraqueza: 'Veneno e Aço',
    descricao : 'A concha parece estar cheia de alegria. Diz-se que ele compartilhará boa sorte quando tratado com gentileza.',
    imagem : 'https://www.pokemon.com/static-assets/content-assets/cms2/img/pokedex/full/175.png',
    link : 'https://www.pokemon.com/br/pokedex/togepi'
}
]